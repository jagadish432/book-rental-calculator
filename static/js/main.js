...
$("#plus").click(function(){
//console.log(row);
$("#myBooks").append('<li class="booksList" style="height: 40px"><div class="book container-fluid"><div style="float:left; margin-left: 10px;">Enter quantity: <input type="number" id="booksQuantity" class="bookQuantity" name="booksQuantity" min="1" minlength="1" onkeypress="return (event.charCode !=8 && event.charCode ==0 || (event.charCode >= 48 && event.charCode <= 57))" /></div><div style="float:left; margin-left: 10px;">Enter duration(in days): <span class="glyphicon glyphicon-question-sign" data-toggle="tooltip" title="round the duration to the next nearest day i.e., duration should be 2 if the number of hours is >24 and <=48 hours"> </span><input type="number" id="dayDuration" class="dayDuration" name="dayDuration" min="1" minlength="1" onkeypress="return (event.charCode !=8 && event.charCode ==0 || (event.charCode >= 48 && event.charCode <= 57))" /></div><a href="javascript:void(0);" onclick="remove(this)"><span class="glyphicon glyphicon-trash"></span></a></div></li>')
});
...
function mysubmit(){
    ...
    for(var i=0; i<booksList.length; i++) {
        ...
        if(thisBook["bookQuantity"] == "" || thisBook["dayDuration"] == ""){
